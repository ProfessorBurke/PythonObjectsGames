...
def main() -> None:
    ...
    while not user_quit:
        ...
        if distance1 < distance2 and distance1 != 0:
            robotx += speed * (battery1x - robotx) / distance1
            roboty += speed * (battery1y - roboty) / distance1
        elif distance2 != 0:
            robotx += speed * (battery2x - robotx) / distance2
            roboty += speed * (battery2y - roboty) / distance2
        if counter == 120:
            counter = 0
            robotx = random.randint(0, screen.get_width() - robot.get_width())
            roboty = random.randint(0, screen.get_height() - robot.get_height())
