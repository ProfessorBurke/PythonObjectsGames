"""Platform scroller with animated sprite."""
...
class Penguin(pygame.sprite.Sprite):
    ...
    def __init__(self, screen: pygame.Surface) -> None:
        ...
        self._flap_images = []
        for i in range(9):
            self._flap_images.append(pygame.image.load("penguin_flap_" + str(i) +".png").convert_alpha())
        self._x = 0
        ...
        self._facing = RIGHT
        self._flap_index = 0
        self._hop_sound = pygame.mixer.Sound("hop.ogg")
    ...
    def update(self, bricks: pygame.sprite.Group) -> None:
        """Flap if in the air."""
        if not self.on_ground(bricks):
            self.image = self._flap_images[self._flap_index]
            if self._facing == LEFT:
                self.image = pygame.transform.flip(self._raw_image, True, False)
            self._flap_index += 1
            self._flap_index %= len(self._flap_images)
        else:
            if self._facing == LEFT:
                self.image = pygame.transform.flip(self._raw_image, True, False)
            else:
